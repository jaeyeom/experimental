...
This directory contains linting tools for various file formats, including org-lint for Org-mode files and a Go linter for unnecessary interface assertions.
...
- Go 1.16 or later
- Emacs with org-mode (ELPA ~org~ and ~org-contrib~) and ~dart-mode~ for
  org-babel source block linting
- ~$GOPATH/bin~ or ~$GOBIN~ in your PATH

Install Emacs packages via Ansible (~./ensure.sh org-lint~ in
~devtools/setup-dev/ansible/~, or ~ansible-playbook setup-org-lint-emacs.yml~).
...
** unnecessary-interface-assertion-linter

Flags compile-time interface assertions of the form ~var _ Iface = (*T)(nil)~
when the interface has a single implementation and is not used elsewhere.

*** Installation

#+BEGIN_SRC bash
go install github.com/jaeyeom/experimental/devtools/linters/cmd/unnecessary-interface-assertion-linter@latest
#+END_SRC
...
# JSON output (used by devcheck)
unnecessary-interface-assertion-linter --json ./...

# Load packages from a specific directory
unnecessary-interface-assertion-linter --dir path/to/module ./...
#+END_SRC

Exit code 1 if issues are found, 0 if clean.

#+BEGIN_SRC bash
bazel build //devtools/linters/cmd/unnecessary-interface-assertion-linter
#+END_SRC
