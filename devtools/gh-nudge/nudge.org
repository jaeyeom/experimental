...
repeated reminders for the same outstanding review request. The notification
history is persisted to a JSON file between runs:
...
scheduled cron job), reviewers will only be notified about the same outstanding
review request once per `reminder_threshold_hours` period.

The cooldown is per request cycle, not per reviewer for the life of the PR. If a
reviewer submits a review and is later re-requested, the previous ping no longer
blocks a new reminder. Dry-run mode still does not record pings.
