...
func TestExecutionResult_Validate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "startTime cannot be zero",
		},
		{
			name: "zero end time",
			er: ExecutionResult{
		...
		},
		{
			...
			wantErr: true,
			errMsg:  "endTime cannot be before startTime",
		},
	}
...
func TestExecutionResult_JSON(t *testing.T) {
	...
	jsonStr := string(data)
	if !strings.Contains(jsonStr, `"duration":"5s"`) {
		t.Errorf("JSON should contain duration field")
	}
	if !strings.Contains(jsonStr, `"startTime":"2024-01-01T12:00:00Z"`) {
		t.Errorf("JSON should contain formatted startTime")
	}

	// Test unmarshaling
	var er2 ExecutionResult
	if err := json.Unmarshal(data, &er2); err != nil {
		t.Fatalf("Failed to unmarshal ExecutionResult: %v", err)
	}

	// Verify fields
	if er2.Command != er.Command {
		t.Errorf("Command mismatch: got %v, want %v", er2.Command, er.Command)
	}
	if er2.ExitCode != er.ExitCode {
		t.Errorf("ExitCode mismatch: got %v, want %v", er2.ExitCode, er.ExitCode)
	}
	if !er2.StartTime.Equal(er.StartTime) {
		t.Errorf("StartTime mismatch: got %v, want %v", er2.StartTime, er.StartTime)
	}
}
