...
immediately. After the send, ~prsync~ records the pane's
~state_change_seq~ (or ~revision~), waits until the agent is observed
~working~ (or the sequence advances without a ~blocked~ flap), then
requires the settle tokens to hold for several consecutive polls.
...
- ~--wait --until idle --until done~ is the herdr send; ~prsync~ then
  waits for a ~working~ (or sequence) transition and a debounced
  settle, not the pre-send ~idle~ status.
