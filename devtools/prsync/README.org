...
| ~dispatch_timeout_ms~ | ~1800000~ | Bound for the per-send settle wait (and ~herdr agent prompt --timeout~). A prompt the agent never picks up becomes ~dispatched_timeout~. |
...
immediately. After herdr returns, ~prsync~ snapshots the pane's
~state_change_seq~ (or ~revision~) and waits until the agent is observed
~working~ (or the sequence advances without a ~blocked~ flap), then
requires the settle tokens to hold for several consecutive polls. Sequence
advances during herdr's own wait (startup ~idle~↔~blocked~ flap) do not
count as a missed ~working~ window.
...
~dispatch_timeout_ms~ (~dispatched_timeout~; state is written so it is
not retried blindly). A ~blocked~ settlement (~dispatched_blocked~)
...
~dispatched~ is a post-prompt ~working~ (or sequence) transition plus a
debounced settle, not the ~idle~ herdr ~--until~ already matched.
...
- ~--wait --until idle --until done~ is the herdr send; ~prsync~ then
  snapshots the pane and waits for a ~working~ (or sequence) transition
  and a debounced settle, not the idle herdr just returned. The live
  gate uses that same debounce so a flap idle is still busy.
