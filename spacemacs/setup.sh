...
# Exclude the symlink from git status so the Spacemacs checkout stays clean.
EXCLUDE_FILE="$CONFIG_DIR/.git/info/exclude"
if ! grep -qxF 'private/local' "$EXCLUDE_FILE" 2>/dev/null; then
	echo 'private/local' >>"$EXCLUDE_FILE"
	echo "Added private/local to .git/info/exclude"
fi
