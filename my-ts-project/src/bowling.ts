...
function assertArrayOfPins(pins: number[]): asserts pins is Pin[] {
  assert(pins.every(isValidPin), "All pins must be integers between 0 and 10.");
}

function assertValidScore(score: number): void {
  assert(Number.isInteger(score), "Invalid pin sequence resulting in a non-integer score.");
  assert(0 <= score && score <= 300, `Invalid score 0 <= ${score} <= 300.`);
}

function assertRemainingThrows(remainingFrames: number, remainingThrows: number): void {
  assert(
    remainingFrames + 1 <= remainingThrows && remainingThrows <= remainingFrames * 2 + 1,
